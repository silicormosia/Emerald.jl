...
"""
    ...
    prescribe!(config::SPACConfig{FT}, spac::BulkSPAC{FT}, df::NamedTuple, ind::Int; initialize_state::Bool = false) where {FT}
...
"""
function prescribe!(config::SPACConfig{FT}, spac::BulkSPAC{FT}, df::NamedTuple, ind::Int; initialize_state::Bool = false) where {FT}
    # read the data out of dataframe row to reduce memory allocation
    df_atm::FT = df.PATM[ind];
    df_chl::FT = df.CHL[ind];
    df_cli::FT = df.CI[ind];
    df_co2::FT = df.CO2[ind];
    df_dif::FT = df.RAD_SW_DIF[ind];
    df_dir::FT = df.RAD_SW_DIR[ind];
    df_doy::FT = df.FDOY[ind];
    df_lai::FT = df.LAI[ind];
    df_lwr::FT = df.RAD_LW[ind];
    df_pcp::FT = df.PPT[ind];
    df_tar::FT = df.TAIR[ind];
    ...
    spac.meteo.rad_sw.e_dir .= view(config.CONSTANTS.SPECTRA.SOLAR_RAD,:,1) .* max(0,df_dir) ./ in_dir;
    spac.meteo.rad_sw.e_dif .= view(config.CONSTANTS.SPECTRA.SOLAR_RAD,:,2) .* max(0,df_dif) ./ in_dif;
