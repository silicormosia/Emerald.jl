...
"""
    ...
    save_fields!(config::SPACConfig{FT}, spac::BulkSPAC{FT}, wdf::NamedTuple, ind::Int; saving_dict::Dict{String,Any} = SAVING_DICT) where {FT}
...
"""
function save_fields!(config::SPACConfig{FT}, spac::BulkSPAC{FT}, wdf::NamedTuple, ind::Int; saving_dict::Dict{String,Any} = SAVING_DICT) where {FT}
    # read the data out of dataframe row to reduce memory allocation
    df_dif::FT = wdf.RAD_SW_DIF[ind];
    df_dir::FT = wdf.RAD_SW_DIR[ind];
    ...
    if saving_dict["ΣSIF"]
        ...
        if isnan(ΣSIF(config, spac))
            @warn "ΣSIF is NaN at index $ind" spac.canopy.sun_geometry.auxil.e_sifꜛ[:,1]
        end;
    end;
